        ...
        <section data-sol="papier" className="border-b bord">
          <div className="max-w-[1400px] mx-auto px-6 md:px-[6vw] py-20 md:py-28">
            {ARTICLES.length === 0 ? (
              ...
              <div className="border bord p-10 md:p-14 max-w-[62ch]">
                <h2 className="h3 sur text-2xl">Les premiers articles arrivent</h2>
                <p className="mt-5 sourd leading-relaxed">
                ...
                <p className="mt-4 sourd leading-relaxed">
                ...
                <Link to="/#contact" className="pilule mono action inline-block mt-8 px-7 py-3.5 bg-encre sur">
                  Poser ma question
                </Link>
              </div>
            ) : (
              <ul className="cascade grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                {ARTICLES.map((a) => (
                  <li key={a.slug} className="border bord hover:bord-fort transition-colors">
                    <Link to={`/blog/${a.slug}`} className="block p-7 h-full flex flex-col">
                      <span className="mono sourd">
                        {new Date(a.date).toLocaleDateString('fr-FR', { year: 'numeric', month: 'long' })} · {a.minutes} min
                      </span>
                      <h2 className="mt-3 h3 sur text-xl">{a.titre}</h2>
                      <p className="mt-3 text-[0.95rem] sourd leading-relaxed grow">{a.resume}</p>
